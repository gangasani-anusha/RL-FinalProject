...
class trainer():
    ...
    def start(self, episodes, stock_type, logging, env_debug, rl_debug, brain, ID, model_based):
        ...
        for eps in self.episodes:
            ...
            if self.brain == 'q':
                ...
                self.operator = DeepQNetwork(
                    self.bike_station.n_actions, self.bike_station.n_features, 0.01, 0.9)
            else:
            ...
            while True:
                ...
                if self.brain == 'q':
                    ...
                    current_hour, old_stock, new_stock, reward, done = self.bike_station.ping_dqn(
                        action)
                    self.operator.store_transition(
                        old_stock, action, reward, new_stock)
                ...
                if brain == 'q':
                    ...
                    self.operator.learn(
                        old_stock, action, reward, new_stock, expected_stock, game_over)
    ...
    def save_session_results(self, timestamp):
        ...
        for session in range(len(self.rewards)):
            ...
            title = "Reward History by Training Session " + \
                str(session) + " - " + timestamp

            x_axis = [x for x in range(self.episodes[session])]
            plt.plot(x_axis, self.rewards[session],
        ...
        for session in range(len(self.session_action_history)):
            ...
            fig = plt.figure(figsize=(10, 8))
            title = "Session " + str(session) + " - Hourly Action of Eps " + \
                str(first_eps_idx) + " and Eps " + str(last_eps_idx)

            x_axis = [x for x in range(
                len(self.session_action_history[session][0]))]
            plt.plot(
                x_axis, self.session_action_history[session][0], label="Eps 0")
            ...
            fig.savefig(file_path + "/action_history_" +
                        str(session) + timestamp)
        ...
        for session in range(len(self.session_stock_history)):
            ...
            title = "[" + self.method + "]" + "Session " + str(
                session) + " - Original vs. Balanced Bike Stock after " + str(first_eps_idx) + " and Eps " + str(last_eps_idx)

            x_axis = [x for x in range(
                len(self.session_stock_history[session][0]))]
            plt.plot(x_axis, self.sim_stock[session],
                     label="Original without Balancing")
            plt.plot(
                x_axis, self.session_stock_history[session][0], label="Balanced Bike Stock - Eps 0")
            ...
            fig.savefig(file_path + "/stock_history_" +
                        str(session) + timestamp)
