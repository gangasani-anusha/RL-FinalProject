...
def process_citibike(starting_bal):
    ...
    bike['starttime'] = pd.to_datetime(
        bike['starttime'], infer_datetime_format=True)
    bike['stoptime'] = pd.to_datetime(
        bike['stoptime'], infer_datetime_format=True)
    ...
    uni_station = pd.concat(
        [uni_dep_stations, uni_arv_stations], axis=0).drop_duplicates()
    uni_station.head()
    ...
    monthDep.columns = ["dep_" + str(day) + "_" + str(hour)
                        for _, day, hour in monthDep.columns]
    ...
    monthArv.columns = ["arv_" + str(day) + "_" + str(hour)
                        for _, day, hour in monthArv.columns]
...
def calHourlyBal(df, starting_bal):
    ...
    final_bal = pd.concat(
        [hourBal[["id", "name", "lat", "lon"]], hourBal[bal_col]], axis=1)
